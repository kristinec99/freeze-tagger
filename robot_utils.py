...
class Robot():
    def __init__(self, players=[], obstacles=[], y=N / 2, x=N / 2, vely=0, velx=0, radius=radius, dt=dt, accelx=accelx,
                 accely=accely, \
    ...
    def Drive(self, players, obstacles, current_players, frozen_players):
        ...
        if not self.target:
            self.target = [random.randrange(0 + self.radius * 2, M - self.radius * 2),
                           random.randrange(0 + self.radius * 2, N - self.radius * 2), 1, -1, 0, 0]
        target = self.target
        ...
        if (x <= 0 + self.radius) or (x > M - self.radius):  # if its going to hit the wall, emergency break
            self.velx = - self.velx
            return (current_players, frozen_players)
        if np.sqrt(np.sqrt((x - target[0]) ** 2 + (y - target[1]) ** 2)) < target[2] and target[
            3] >= 0:  # if robot runs into active player, freeze
            current_players, frozen_players = self.Freeze(target, players, obstacles, current_players, frozen_players)
            return (current_players, frozen_players)

        for obstacle in obstacles:
            if np.sqrt((x - obstacle.x) ** 2 + (
                    y - obstacle.y) ** 2) <= obstacle.radius + self.radius:  # if its going to hit a frozen player bounce back
                self.velx = -self.velx
                self.vely = -self.vely
                return (current_players, frozen_players)
        ...
        counter = 0
        for obstacle in obstacles:
            if self.lineIntersectCircle(obstacle.radius + self.radius, (obstacle.x, obstacle.y), (x, y),
                                        (target[0], target[1])):
                counter = counter + 1
                break
        if counter != 0:
            ...
            while len(targets) <= self.sample:
                target_choice = [random.randrange(max(round(x - self.radius * 5), 0 + self.radius),
                                 ...
                                 random.randrange(max(round(y - self.radius * 5), 0 + self.radius),
                                                  min(round(y + self.radius * 5), M - self.radius)), 1, -1, 0, 0]
                counter = 0
                for obstacle in obstacles:
                    if self.lineIntersectCircle(obstacle.radius + self.radius, (obstacle.x, obstacle.y), (x, y),
        ...
        for player in players:  # list of objects player with params x, y , velx, vely, radius, identifier
            if player.froze == False:
                for k in range(len(self.probProximal)):
                    if np.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2) - player.radius <= \
                            self.probProximal[k][1]:
                        if not closest or np.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2) < np.sqrt(
                                (self.x - closest[0]) ** 2 + (self.y - closest[1]) ** 2):
