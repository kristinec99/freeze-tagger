    ...
    def Drive(self, target):
        ...
        for obstacle in self.obstacles:
            while self.lineIntersectCircle(self.x, target[0], self.y, target[1], obstacle.x, obstacle.y,
                                           obstacle.radius):
                old_target = target
                target = [random.ranrange(min(self.x, old_target[0]) - abs(self.x - old_target[0]) * .25),
                          random.ranrange(min(self.y, old_target[1]) - abs(self.y - old_target[1]) * .25)]
    ...
    def Sensor(self, players):
        ...
        for player in self.players:  # list of objects player with params x, y , velx, vely, radius, identifier
            for k in range(len(self.probProximal)):
                if np.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2) - player.radius <= self.probProximal[k][
                    1]:
                    if not closest or np.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2) < closest[3]:
                        closest = [player.x, player.y, player.radius, player.identifier,
                                   np.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2)]
        return closest[:4]
