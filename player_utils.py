...
class Player():
    def __init__(self, identifier, players=[], obstacles=[], y=random.randrange(0, N), x=random.randrange(0, M),
                 vely=.1, velx=.1,
        ...
        self.target = [random.randrange(0 + self.radius * 2, M - self.radius * 2),
                       random.randrange(0 + self.radius * 2, N - self.radius * 2)]
    ...
    def Walk(self, players, obstacles, current_players, robot, frozen_players):
        ...
        if self.froze:
            if np.sqrt((self.x - robot.x) ** 2 + (
                    self.y - robot.y) ** 2) > self.radius + robot.radius and not self.obstacled:
        ...
        obstructions.append(robot)

        # Once we get close to the target, pick a new one to move towards
        if self.pointIntersectCircle(x, y, self.target[0], self.target[1], self.radius):
            self.target = [random.randrange(0 + self.radius * 2, M - self.radius * 2),
                           random.randrange(0 + self.radius * 2, N - self.radius * 2)]
            self.target_identifier = -100
            if frozen_players and random.choice([1, 2]) == 1:
                # tag closest player
                dist = 1000
        ...
        for obstacle in obstructions:  # if its going to hit another player or obstacle, bounce back
            if self.pointIntersectCircle(x, y, obstacle.x, obstacle.y,
                                         obstacle.radius + self.radius) and obstacle.identifier != self.identifier:
                if obstacle.identifier >= 0 and obstacle.froze == True:
                    current_players, frozen_players = self.Unfreeze(obstacle.identifier, obstacles, current_players,
                                                                    frozen_players)
                    return (current_players, frozen_players)
        ...
        for obstacle in obstructions:
            if obstacle.identifier not in [self.identifier, self.target_identifier] and self.lineIntersectCircle(
                    obstacle.radius + self.radius, (obstacle.x, obstacle.y), (x, y), (self.target[0], self.target[1])):
                counter = counter + 1
                break
        target = self.target
        if counter != 0:
            ...
            while len(targets) <= self.sample:
                target_choice = [random.randrange(max(round(x - self.radius * 5), 0 + self.radius),
                                                  min(round(x + self.radius * 5), N - self.radius)),
                                 random.randrange(max(round(y - self.radius * 5), 0 + self.radius),
                                                  min(round(y + self.radius * 5), M - self.radius))]
                counter = 0
                for obstacle in obstructions:
                    if obstacle.identifier not in [self.identifier,
                                                   self.target_identifier] and self.lineIntersectCircle(
                        obstacle.radius + self.radius, (obstacle.x, obstacle.y), (x, y),
                        (target_choice[0], target_choice[1])):
                        counter = counter + 1
                        break
                if counter == 0:
            ...
            for target_choice in targets:
                if ((self.target[0] - target_choice[0]) ** 2 + (self.target[1] - target_choice[1]) ** 2) < (
                        (self.target[0] - target[0]) ** 2 + (self.target[1] - target[1]) ** 2):
                    target = target_choice
        self.accelx = (2 / (T * 2) * (0 - self.velx) + 1 / (T * 2) ** 2 * (target[0] - self.x))
        self.accely = (2 / (T * 2) * (0 - self.vely) + 1 / (T * 2) ** 2 * (target[1] - self.y))
