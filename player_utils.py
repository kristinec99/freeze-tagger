...
class Robot():
    def __init__(self, obstacles, y = 25, x = 25, vely = 0, velx = 0, radius = .5, dt = .1, accel = 1
                 probCmd = 1.0, probProximal = [1.0, 10]):
        ...
        while True:
            ...
            for obstacle in self.obstacles:
                if np.sqrt((self.x-obstacle[0])^2+(self.y-obstacle[1])^2) <= obstacle[2]:
                    counter = counter + 1
            if counter == 0:
                break
    ...
    def Drive(self, target):
        ...
        for obstacle in self.obstacles:
            while self.lineIntersectCircle(self.x, target[0], self.y, target[1], obstacle[0], obstacle[1], obstacle[2]):
            ...
            for k in range(len(self.probProximal)):
                if np.sqrt((self.x-players.x)^2+(self.y-player.y)^2) - player.radius <= self.probProximal[k][1]:
                    if not closest or np.sqrt((self.x-player.x)^2+(self.y-player.y)^2) < closest[3]:
                        closest =[player[0], player[1], player[2], player[3] np.sqrt((self.x-players[0])^2+(self.y-players[1])^2)]
        return closest[:3]
    def lineIntersectCircle (x1, x2, y1, y2, cx, cy, r):
        return (abs((x2-x1)*cx + (y1-y2)*cy + (x1-x2)*y1 + (y2-y1)*x1) / np.sqrt((x2-x1)^2 +(y2 - y1)^2) <= r)
